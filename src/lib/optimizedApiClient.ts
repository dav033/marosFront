...
export class OptimizedApiClient {
   ...
   */
  async get<T = any>(url: string, config: CachedRequestConfig = {}): Promise<AxiosResponse<T>> {
    const startTime = Date.now();
    this.metrics.totalRequests++;

    const cacheKey = this.generateCacheKey('GET', url, config.params);
    const cacheConfig = { enabled: true, strategy: 'cache-first' as const, ...config.cache };
    ...
    try {
      ...
      if (cacheConfig.enabled && cacheConfig.strategy === 'network-first') {
        const cached = this.getCachedResponse<T>(cacheKey);
        if (cached) {
          console.warn(`Network failed for ${url}, returning cached data:`, error);
          return cached;
        }
      }
   ...
   */
  async post<T = any>(url: string, data?: any, config: CachedRequestConfig = {}): Promise<AxiosResponse<T>> {
    const startTime = Date.now();
    this.metrics.totalRequests++;
    this.metrics.networkRequests++;
   ...
   */
  async put<T = any>(url: string, data?: any, config: CachedRequestConfig = {}): Promise<AxiosResponse<T>> {
    const startTime = Date.now();
    this.metrics.totalRequests++;
    this.metrics.networkRequests++;
   ...
   */
  async delete<T = any>(url: string, config: CachedRequestConfig = {}): Promise<AxiosResponse<T>> {
    const startTime = Date.now();
    this.metrics.totalRequests++;
    this.metrics.networkRequests++;
  ...
  async prefetch(url: string, config: CachedRequestConfig = {}): Promise<void> {
    ...
    try {
      ...
      const prefetchId = prefetchManager.register(
        cacheKey,
        () => this.get(url, { ...config, cache: { enabled: true } }).then(r => r.data),
        {
   ...
   */
  getMetrics(): RequestMetrics & { cache: any; prefetch: any } {
    return {
      ...this.metrics,
  ...
  private async makeRequest<T>(
    ...
    const retryConfig = { enabled: true, attempts: 3, delay: 1000, backoff: 'exponential' as const, ...config.retry };

    let lastError: any;
    
    for (let attempt = 0; attempt < (retryConfig.enabled ? retryConfig.attempts : 1); attempt++) {
      try {
  ...
  private setCachedResponse<T>(key: string, response: AxiosResponse<T>, ttl?: number): void {
    ...
    for (const [pattern, regexes] of Object.entries(patterns)) {
      if (baseUrl.includes(pattern)) {
        regexes.forEach(regex => {
          ...
          if (invalidated > 0) {
            console.debug(`Invalidated ${invalidated} cache entries for pattern ${regex}`);
          }
        });
      }
    }
...
export const optimizedApiClient = new OptimizedApiClient('http://localhost:8080');
