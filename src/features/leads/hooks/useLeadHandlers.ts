...
import { LeadAgent } from "../agent/LeadAgent";
import { LeadEventBus } from "../agent/LeadEvents";
...
export function useLeadHandlers(refetchLeads: () => void) {
  ...
  const handleLeadCreated = useCallback(async (newLead: Omit<Lead,"id">) => {
    await agent.create(newLead);
  }, [agent]);

  const handleLeadUpdated = useCallback(async (lead: Lead) => {
